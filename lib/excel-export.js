...
const ExcelExport = (function () {
  ...
  /**
   * 导出 SVN 版本验证记录为 Excel。
   * 列：FSG TAG / SW / RF / Date / 客户版本 / Sheet / 结果 / 不匹配项 / SVN版本号 / 检查时间
   */
  async function exportSvnRecords(records) {
    if (!records || records.length === 0) {
      throw new Error('没有可导出的记录');
    }
    if (typeof ExcelJS === 'undefined') {
    ...
    const headers = ['FSG TAG', '软件版本SW', '射频版本RF', '日期Date',
      '客户版本', '表格Sheet', '结果', '不匹配/异常信息', 'SVN版本号', '检查时间'];

    const workbook = new ExcelJS.Workbook();
    workbook.creator = 'NvCheckList Web';
    workbook.created = new Date();
    const ws = workbook.addWorksheet('SVN版本验证', { properties: { defaultRowHeight: ROW_HEIGHT } });
    ...
    records.forEach((rec, idx) => {
      ...
      const pass = rec.overall === 'pass';
      const vals = [
        rec.tag || rec.module || '',
        rec.sw || '', rec.rf || '', rec.date || '',
        rec.customer || '', rec.sheet || '',
        pass ? 'PASS' : 'FAIL',
        rec.fail_text || '',
        rec.svn_revision ? ('r' + rec.svn_revision) : '',
      ...
      vals.forEach((v, ci) => {
        ...
        cell.value = v;
        cell.font = { ...cellBase };
        cell.alignment = (ci === 7) ? left : center;
        cell.border = border;
        // 结果列着色
        if (ci === 6) {
          cell.font = { ...cellBase, bold: true, color: { argb: pass ? 'FF006100' : 'FF9C0006' } };
          cell.fill = {
            type: 'pattern', pattern: 'solid',
            fgColor: { argb: pass ? 'FFC6EFCE' : 'FFFFC7CE' },
          };
        }
        if (v && String(v).split('\n').length > maxLines) maxLines = String(v).split('\n').length;
      });
      row.height = Math.max(ROW_HEIGHT, 15 * maxLines);
    });
    ...
    for (let c = 1; c <= headers.length; c++) {
      ...
      for (let r = 2; r <= records.length + 1; r++) {
        const v = ws.getCell(r, c).value;
        if (v != null) {
          String(v).split('\n').forEach(ln => { w = Math.max(w, charWidth(ln)); });
        }
      }
      ws.getColumn(c).width = Math.min(Math.max(w + 3, 10), 50);
    }
    ...
    const fname = 'SvnVersionCheck_' + formatFileStamp(new Date()) + '.xlsx';
    triggerDownload(blob, fname);
    return fname;
  }
  ...
  return { exportRecords, exportSvnRecords };
})();
