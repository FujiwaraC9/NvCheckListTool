...
const ConfigManager = (function () {
  ...
  const FALLBACK = {
    ...
    dimensions: [
      ...
      { key: 'baseline', label: '基线', order: 5, options: ['UIS7885->5G_MODEM_V2_23A_IEBU_W24.31.2'], auto_read: true },
    ],
    items: [
      { name: 'edch_Category', default: '0x7', nvm_file: '', conditions: {} },
      { name: 'gea_encryption_algo1', default: '0x0', nvm_file: '', conditions: {}, note: 'load设备中的NV进行check，代码修复（客户分支未关闭，待有客户需求再导入）' },
   ...
   * @param dims   维度数组（用于判断 auto_read）
   * @returns boolean
   *
   * 规则：
   * - conditions 中的键 = 该维度必须匹配
   * - 非 auto_read 维度（如 customer/nv_type）值为空时按"未选择"处理，不匹配（避免误检）
   * - auto_read 维度（platform/android_version/baseline）值为空时：
   *     - 若该维度在 conditions 里有限制 → 不匹配（等自动读取或手选）
   *     - 若 conditions 里没限制 → 不影响
   * - 选中值 ∈ conditions[dimKey] 数组才匹配
   */
  function matchItem(item, sel, dims) {
  ...
  function getActiveItems(sel) {
    ...
    return items.filter(item => matchItem(item, sel, dims));
  }
