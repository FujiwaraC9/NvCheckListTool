...
const ConfigManager = (function () {
  ...
  // 云端配置源：GitHub 优先（用户改了 GitHub 后能立刻拉到），本地文件兜底
  const CLOUD_URLS = [
  ...
  const FALLBACK = {
    ...
    dimensions: [
      ...
      { key: 'customer', label: '客户版本', order: 3, auto_read: null, options_by: { '9': ['公版'], '10': ['公版', '飞天'], '12': ['公版', '新大陆定制'] } },
  ...
  async function fetchCloudConfig() {
    ...
    for (const url of CLOUD_URLS) {
      try {
        ...
        const resp = await fetch(url + sep + '_t=' + ts, { cache: 'no-cache' });
        if (resp.ok) {
