...
const Serial = (function () {
  ...
  async function requestPort(keyword) {
    ...
    try {
      // 完全不传任何参数给原生 API，保留浏览器默认的所有行为（显示完整的 COM 友好名等）
  ...
  /**
   * 从 AT+QGMR 响应中提取安卓版本号。
   * 最宽松匹配：找到 "Android"（忽略大小写），跳过其后任意非数字字符，取第一个数字序列作为主版本号。
   * @returns {string|null}  '9'/'10'/'12'/...
   */
  function parseAndroidVersion(respText, versionOptions) {
    if (!respText) return null;
    // 先清洗控制字符（NUL/DEL/CR/LF 等串口可能夹带的不可见字节）
    const cleaned = respText.replace(/[\x00-\x1F\x7F]/g, '');
    // 最宽松：Android 后任意非数字字符都跳过，直接取第一个数字串
    const idx = cleaned.toLowerCase().indexOf('android');
    if (idx >= 0) {
      const after = cleaned.substring(idx + 7); // "android".length === 7
      const m = after.match(/(\d+)/);
      if (m) {
        const v = m[1];
        console.log('[parseAndroidVersion] raw=' + JSON.stringify(respText.substring(0, 200)) + ' cleaned=' + JSON.stringify(cleaned.substring(0, 200)) + ' version=' + v);
        if (versionOptions && versionOptions.length > 0) {
          return versionOptions.includes(v) ? v : v;
        }
        return v;
      }
    }
    console.log('[parseAndroidVersion] FAILED raw=' + JSON.stringify(respText.substring(0, 300)));
    return null;
  }
