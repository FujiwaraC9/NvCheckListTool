...
const Serial = (function () {
   ...
   * QGMR 输出中可能夹有串口空字节/控制字符（如 _Android\x0012），先清洗再匹配。
   * 匹配模式：Android（可前后接非字母字符）+ 数字主版本号
   * @returns {string|null}  '9'/'10'/'12'/...
   */
  function parseAndroidVersion(respText, versionOptions) {
    if (!respText) return null;
    // 清洗：去掉 NUL 及其他控制字符（保留可见 ASCII、中文、常见标点）
    const cleaned = respText.replace(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/g, '');
    // 匹配 Android 后面紧跟的主版本号（允许下划线/点/空白等非字母数字作为分隔，如 _Android12、Android 12、Android.12）
    const m = cleaned.match(/Android[_\s.]*(\d+)/i);
    if (m) {
      const v = m[1];
